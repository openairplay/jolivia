		...
		if(channels == 0) // 1 channel
		{
			...
			{ // so it is compressed
				...
				else
				{
					...
					/*
					 * i think the only other prediction type (or perhaps this is just a boolean?) runs adaptive fir twice.. like: predictor_decompress_fir_adapt(predictor_error, tempout, ...) predictor_decompress_fir_adapt(predictor_error, outputsamples ...) little strange..
					 */
				}
			...
			{ // not compressed, easy case
				...
				else
				{
					...
					for(int i = 0; i < outputsamples; i++)
					{
						...
						/*
						 * special case of sign extension.. as we'll be ORing the low 16bits into this
						 */
