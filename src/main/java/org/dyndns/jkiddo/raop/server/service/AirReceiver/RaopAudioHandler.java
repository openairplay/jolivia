...
public class RaopAudioHandler extends SimpleChannelUpstreamHandler
{
	...
	/**
	 * Routes incoming packets from the control and timing channel to the audio channel
	 */
	private class RaopRtpInputToAudioRouterUpstreamHandler extends SimpleChannelUpstreamHandler
	{
		...
		public void messageReceived(final ChannelHandlerContext ctx, final MessageEvent evt) throws Exception
		{
			...
			{
				audioChannel.getPipeline().sendUpstream(new UpstreamMessageEvent(audioChannel, evt.getMessage(), evt.getRemoteAddress()));
			}
		}
	}

	/**
	 * Routes outgoing packets on audio channel to the control or timing channel if appropriate
	 */
		...
		for(final String line : dsp.split("\n"))
		{
			...
			switch(attribute)
			{
				...
				case 'a':
					...
					{
						/*
						 * Sets the AES key required to decrypt the audio data. The key is encrypted wih the AirTunes private key
						 */
						byte[] aesKeyRaw;
	...
	/**
	 * Handles RECORD request. We did all the work during ANNOUNCE and SETUP, so there's nothing more to do. iTunes reports the initial RTP sequence and playback time here, which would actually be helpful. But iOS doesn't, so we ignore it all together.
	 */
	public synchronized void recordReceived(final ChannelHandlerContext ctx) throws Exception
	{
	...
	/**
	 * Handle FLUSH requests. iTunes reports the last RTP sequence and playback time here, which would actually be helpful. But iOS doesn't, so we ignore it all together.
	 */
	private synchronized void flushReceived(final ChannelHandlerContext ctx)
	{
		if(m_audioOutputQueue != null)
	 ...
	 */
	private synchronized void teardownReceived(final ChannelHandlerContext ctx)
	{
		final HttpResponse response = new DefaultHttpResponse(RtspVersions.RTSP_1_0, RtspResponseStatuses.OK);
	 ...
	 */
	public synchronized void getParameterReceived(final ChannelHandlerContext ctx)
	{
	 ...
	 * @param channelType
	 *            channel type. Determines which handlers are put into the pipeline
	...
	private Channel createRtpChannel(final SocketAddress local, final SocketAddress remote, final RaopRtpChannelType channelType)
	{
		...
		/*
		 * Set the buffer size predictor to 1500 bytes to ensure that received packets will fit into the buffer. Packets are truncated if they are larger than that!
		 */
		...
		bootstrap.setPipelineFactory(new ChannelPipelineFactory() {
			...
			public ChannelPipeline getPipeline() throws Exception
			{
				...
				/*
				 * We pretend that all communication takes place on the audio channel, and simply re-route packets from and to the control and timing channels
				 */
	...
	/**
	 * Modifies the port component of an {@link InetSocketAddress} while leaving the other parts unmodified.
	 * 
	 * @param address
	 *            socket address
	 * @param port
	...
	{
		/*
		 * The more natural way of doing this would be new InetSocketAddress(address.getAddress(), port), but this leads to a JVM crash on Windows when the new socket address is used to connect() an NIO socket. According to http://stackoverflow.com/questions/1512578/jvm-crash-on-opening-a-return-socketchannel converting to address to a string first fixes the problem.
		 */
