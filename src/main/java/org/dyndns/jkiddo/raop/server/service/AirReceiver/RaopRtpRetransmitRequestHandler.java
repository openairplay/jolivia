...
package org.dyndns.jkiddo.raop.server.service.AirReceiver;
 ...
 * <p>
 * Sends {@link RaopRtpPacket.RetransmitRequest} packet in response to missing packets, and resends those requests after a timeout period until the packet arrives.
 * <p>
 * Uses an {@link AudioClock} as it's time source, any thus only re-requests packets which can reasonably be expected to arrive before their play back time.
 */
public class RaopRtpRetransmitRequestHandler extends SimpleChannelUpstreamHandler
{
	...
	private class MissingPacket
	{
		...
		/**
		 * Creates a MissingPacket instance for a given sequence, using the provided time to compute the times at which the packet is expected.
		 * 
		 * @param _sequence
		 *            sequence number
		 * @param nextSecondsTime
		 *            next possible play back time
		 */
	...
	/**
	 * Frames per packet. Used to interpolate the RTP time stamps of missing packets.
	 */
	...
	/**
	 * RTP frame time corresponding to packet with the latest sequence number.
	 */
	...
	/**
	 * Mark the packet as retransmitted, i.e. remove it from the list of in-flight retransmit requests.
	 * 
	 * @param sequence
	...
	/**
	 * Mark the packet is missing, i.e. add an entry to the list of in-flight retransmit requests.
	 * 
	 * @param sequence
	 *            sequence number of packet
	 * @param nextSecondsTime
	...
	private void markMissing(final int sequence, final double nextSecondsTime)
	{
		...
		/*
		 * Forget about old missing packets if we exceeded the number of in-flight retransmit requests
		 */
	...
	/**
	 * Scan the list of in-flight retransmit requests and send {@link RetransmitRequest} packets if it's past the time at which we expected the packet to arrive
	 * 
	 * @param channel
	 *            channel used to send retransmit requests
	 * @param nextSecondsTime
	 */
	private synchronized void requestRetransmits(final Channel channel, final double nextSecondsTime)
	{
		...
		while(missingPacketIterator.hasNext())
		{
			...
			/*
			 * If it's past the time at which the packet would have needed to be queued, warn and forget about it
			 */
			if(missingPacket.requiredUntilSecondsTime <= nextSecondsTime)
			{
			...
			/*
			 * If the packet isn't expected until later, skip it for now
			 */
			...
			if((retransmitRequest != null) && (sequenceAdd(retransmitRequest.getSequenceFirst(), retransmitRequest.getSequenceCount()) != missingPacket.sequence))
			{
				/*
				 * There is a current retransmit request, but the sequence cannot be appended. We transmit the current request and start building a new one
				 */
	...
	public void messageReceived(final ChannelHandlerContext ctx, final MessageEvent evt) throws Exception
	{
		if(evt.getMessage() instanceof RaopRtpPacket.AudioTransmit)
			audioTransmitReceived((RaopRtpPacket.AudioTransmit) evt.getMessage());
		else if(evt.getMessage() instanceof RaopRtpPacket.AudioRetransmit)
			audioRetransmitReceived((RaopRtpPacket.AudioRetransmit) evt.getMessage());
	...
	private synchronized void audioRetransmitReceived(final RaopRtpPacket.AudioRetransmit audioPacket)
	{
	...
	private synchronized void audioTransmitReceived(final RaopRtpPacket.AudioTransmit audioPacket)
	{
		final double nextSecondsTime = m_audioClock.getNextSecondsTime();

		/*
		 * Mark packet as retransmitted. Doing this here prevents sending out further retransmit requests for packets which simply were delayed
		 */
		...
		/*
		 * Remember the sequence we expected, then update the latest received sequence and it's frame time iff the new sequence is larger than the old one
		 */
	...
	/**
	 * Returns the number of sequences between from and to, including to.
	 * 
	 * @param from
	 *            first sequence
	 * @param to
	...
	/**
	 * Returns the difference between to sequences. Since sequences are circular, they're not totally ordered, and hence it's ambiguous whether the delta is positive or negative. We return the number with the smaller <b>absolute</b> value.
	 * 
	 * @param a
	 *            first sequence
	 * @param b
	...
	/**
	 * Adds a delta to a given sequence and returns the resulting sequence.
	 * 
	 * @param seq
	 *            sequence
	 * @param delta
