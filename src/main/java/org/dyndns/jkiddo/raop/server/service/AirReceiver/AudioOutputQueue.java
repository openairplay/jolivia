...
public class AudioOutputQueue implements AudioClock
{
	...
	/**
	 * True if the line's audio format is signed but the requested format was unsigned
	 */
	private final boolean m_convertUnsignedToSigned;

	/**
	 * Bytes per frame, i.e. number of bytes per sample times the number of channels
	 */
	...
	/**
	 * Average packet size in frames. We use this as the number of silence frames to write on a queue underrun
	 */
	...
	/**
	 * The last frame written to the line. Used to generate filler data
	 */
			...
			try
			{
				...
				while(!m_closing)
				{
					if(!m_queue.isEmpty())
					{
						...
						/*
						 * If the gap between the next packet and the end of line is negligible (less than one packet), we write it to the line. Otherwise, we fill the line buffer with silence and hope for further packets to appear in the queue
						 */
				...
				/*
				 * Before we exit, we fill the line's buffer with silence. This should prevent noise from being output while the line is being stopped
				 */
