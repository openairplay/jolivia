...
public class Jolivia {
	...
	public static void main(String[] args) throws Exception {
		...
		new Jolivia(new IImageStoreReader() {
	...
	class JoliviaServlet extends HttpServlet {
		...
		public JoliviaServlet() {
			try {
				daap = new DAAPResource(JmmDNS.Factory.getInstance(), 4000,
						"Jolivia", new MusicItemManager("Jolivia",
		...
		protected void doGet(HttpServletRequest httpServletRequest,
			...
			try {
				if (httpServletRequest.getPathInfo().startsWith("/server-info"))
					response = daap.serverInfo(httpServletRequest,
							httpServletResponse);
				else if (httpServletRequest.getPathInfo().startsWith("/login"))
					response = daap.login(httpServletRequest,
							httpServletResponse);
				else if (httpServletRequest.getPathInfo().startsWith("/update"))
					response = daap.update(
							httpServletRequest,
							httpServletResponse,
							getParameterAsInteger("session-id",
									httpServletRequest),
							getParameterAsInteger("revision-number",
									httpServletRequest),
							getParameterAsInteger("delta", httpServletRequest),
							getParameterAsInteger("daap-no-disconnect",
									httpServletRequest));
				else if (httpServletRequest.getPathInfo().startsWith(
						"/databases")
						&& httpServletRequest.getPathInfo().endsWith("items")
						&& httpServletRequest.getPathInfo().contains(
								"containers"))
					response = daap.containerItems(
							...
							getParameterAsInteger("revision-number",
									httpServletRequest),
							getParameterAsInteger("delta", httpServletRequest),
							getParameterAsString("meta", httpServletRequest),
							getParameterAsString("type", httpServletRequest),
							getParameterAsString("group-type",
									httpServletRequest),
							getParameterAsString("sort", httpServletRequest),
							getParameterAsString("include-sort-headers",
									httpServletRequest),
							getParameterAsString("query", httpServletRequest),
							getParameterAsString("index", httpServletRequest));
				else if (httpServletRequest.getPathInfo().startsWith(
						"/databases")
						&& httpServletRequest.getPathInfo().endsWith("items"))
					response = daap.items(
						...
						"/databases")
						&& httpServletRequest.getPathInfo().endsWith(
								"containers"))
					response = daap.containers(
							httpServletRequest,
							httpServletResponse,
							0,
							getParameterAsInteger("session-id",
				...
				if (response != null) {
					...
					MultivaluedMap<String, Object> meta = response
							.getMetadata();
					Set<String> keys = meta.keySet();
					for (String key : keys) {
						httpServletResponse.setHeader(key,
								(String) meta.getFirst(key));
					}
		...
		private int getParameterAsInteger(String string,
		...
		private String getParameterAsString(String string,
				HttpServletRequest httpServletRequest) {
