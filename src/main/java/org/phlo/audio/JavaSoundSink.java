		...
		public void run()
		{
			try
			{
				...
				while(!exit)
				{
					...
					if(sampleSourceBuffer != null)
					{
						silenceSamples = (int) Math.round(Math.min(Math.max(0.0, (sampleSourceBuffer.getTimeStamp() - m_lineEndTime) * m_sampleRate), (double) m_silence.getDimensions().samples));
						skipSamples = (int) Math.round(Math.min(Math.max(0.0, (m_lineEndTime - sampleSourceBuffer.getTimeStamp()) * m_sampleRate), (double) sampleSourceBuffer.getDimensions().samples));
					}
					else
					{
						silenceSamples = (int) Math.ceil(Math.min((m_startTime - m_lineEndTime) * m_sampleRate, (double) m_silence.getDimensions().samples));
						skipSamples = -1;
					}
