 ...
 * <p>
 * Used as a factory for {SampleAccessor} instances which provide access to the buffer's samples as float values indexed by their absolute position inside the buffer.
 */
public enum SampleByteFormat
{
	...
	{
		@Override
		public final Signedness getSignedness()
		{
			return Signedness.Unsigned;
		}

		@Override
		public final SampleAccessor getAccessor(final ByteBuffer buffer)
		{
			return new SampleAccessor() {
				@Override
				public float getSample(final int index)
				{
	...
	{
		if((AudioFormat.Encoding.PCM_SIGNED.equals(audioFormat.getEncoding())) && (audioFormat.getSampleSizeInBits() == 16))
		{
			return SignedInteger16;
		}
		else if((AudioFormat.Encoding.PCM_UNSIGNED.equals(audioFormat.getEncoding())) && (audioFormat.getSampleSizeInBits() == 16))
		{
	...
	/**
	 * Returns the size required to store a buffer of the given {@code dimensions}.
	 * 
	 * @param dimensions
	...
	/**
	 * Computes the number of samples in a buffer from it's size in bytes and the number of channels.
	 * 
	 * @param channels
	 *            number of channels
	 * @param byteSize
	...
	/**
	 * Returns the signedness (signed or unsigned) of a sample byte format
	 * 
