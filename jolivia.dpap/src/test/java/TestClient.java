...
public class TestClient
{
	...
	class DPAPClient
	{
		...
		public DPAPClient(final String host, final int port) throws Exception
		{
			...
			final LoginResponse loginResponse = RequestHelper.requestParsed(String.format("%s/login", this.getRequestBase()));
			...
			final ServerDatabases serverDatabases = RequestHelper.requestParsed(String.format("%s/databases?session-id=%s", this.getRequestBase(), this.sessionId));
			...
			// Local database
			{
				...
				localDatabase.getSpecificChunk(ItemName.class).getValue();
				final int itemId = localDatabase.getSpecificChunk(ItemId.class).getValue();
				localDatabase.getSpecificChunk(PersistentId.class).getUnsignedValue().longValue();

				// fetch playlists to find the overall magic "Music" playlist
				final DatabaseContainerns allPlaylists = RequestHelper.requestParsed(String.format("%s/databases/%d/containers?session-id=%s&meta=dmap.itemname,dmap.itemcount,dmap.itemid,dmap.persistentid,daap.baseplaylist,com.apple.itunes.special-playlist,com.apple.itunes.smart-playlist,com.apple.itunes.saved-genius,dmap.parentcontainerid,dmap.editcommandssupported", this.getRequestBase(), itemId, this.sessionId));

				allPlaylists.getListing().getSingleListingItemContainingClass(BaseContainer.class);
		...
		{
			final ServerInfoResponse serverInfoResponse = RequestHelper.requestParsed(String.format("%s/server-info", this.getRequestBase()));
			return serverInfoResponse;
		}

		public DataControlInt getControlInt() throws Exception
		{
			final DataControlInt serverInfoResponse = RequestHelper.requestParsed(String.format("%s/ctrl-int", this.getRequestBase()));
			return serverInfoResponse;
		}
