...
import org.dyndns.jkiddo.raop.server.audio.SampleSource;
		...
		JavaSoundSink sink = new JavaSoundSink(44100, 2, new SampleSource() {
			...
			public SampleBuffer getSampleBuffer(double timeStamp) {
				...
				for(int i=0; i < sine.getDimensions().samples; ++i) {
					sine.setSample(0, i, (float)Math.sin(2.0 * Math.PI * frequencyLeft * (timeStamp + i / sampleRate)));
					sine.setSample(1, i, (float)Math.sin(2.0 * Math.PI * frequencyRight * (timeStamp + i / sampleRate)));
				}
